...
train_indice = np.random.choice(np.arange(variation), int(variation*0.8), replace=False)
test_indice = np.setdiff1d(np.arange(variation), train_indice)
...
for i, indice in enumerate([train_indice, test_indice, valid_indice]):
    ...
    for variation_number in indice:
